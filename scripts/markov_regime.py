...
# --------------------------------------------------------------------------- #
def clean_futures_rolls(close: pd.Series, max_daily_move: float = 0.10) -> pd.Series:
    """Neutralize roll artifacts in continuous futures price series.

    Futures continuous contracts stitch together expiring front-month contracts,
    causing price jumps at roll dates that don't reflect real market moves. These
    are typically 1–5% (the basis between contracts) but occasionally larger.

    We detect days where the single-day move exceeds max_daily_move (10% default),
    zero out those returns, then reconstruct a clean price series. The overall
    price trend is preserved; only the roll-day spikes are smoothed.

    Crypto is excluded — 20%+ single-day moves are genuine there.
    """
...
def analyze(
    ...
    *,
    ...
    # Auto-clean roll artifacts for futures (tickers ending in =F).
    # Crypto excluded — 20%+ single-day moves are genuine there.
    rolls_cleaned = 0
    if source.endswith("=F"):
        close, rolls_cleaned = clean_futures_rolls(close, max_daily_move=0.10)
    ...
    # Count consecutive days at the end sharing the current regime
    arr_labels = np.asarray(labels, dtype=int)
    regime_duration = 1
    for i in range(len(arr_labels) - 2, -1, -1):
        if arr_labels[i] == current_state:
            regime_duration += 1
        else:
            break
    ...
    return {
        ...
        "futures_rolls_cleaned": rolls_cleaned,
